...
import bulletpointsTick from '../media/bulletpointsTick.svg';
import arrowUpmehrInfo from '../media/arrowUpmehrInfo.svg';
import arrowDownmehrInfo from '../media/arrowDownmehrInfo.svg';
...
const Card = ({ cardData }) => {
  ...
  return (
    ...
    <h1>Kreditkarten-Vergleich: Top Kreditkarten 2024</h1>
          ...
          <div className="credit-card__details">
            <ul className="credit-card__features">
              {item.feature.map((feature, index) => (
                <li key={index} className="credit-card__feature"><img src={bulletpointsTick} className='bulletpointsTick' alt='tick' /><span>{feature}</span></li>
              ))}
            </ul>
            <div className="credit-card__annual-fee"><span>{item.KartengebührProJahr}</span><br /><p>Grundgebühr</p></div>
          </div>
          <div className="credit-card__actions">
            ...
            >
              {dropdownOpen && activeCardIndex === index ? (
                <>
                  <span>Weniger Info</span>
                  <img src={arrowUpmehrInfo} className='arrowUpInfo' alt='arrow-up' />
                </>
              ) : (
                <>
                  <span>Mehr Info</span>
                  <img src={arrowDownmehrInfo} className='arrowDownInfo' alt='arrow-down' />
                </>
              )}
            </button>
          </div>
